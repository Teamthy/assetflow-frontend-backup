...
export function AdminDashboard() {
  ...
  const { data: auditData, isLoading: auditLoading } = useQuery({
    ...
    queryFn: async () => {
      const payload = await assetApi.audit()
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard label="Active Branches" value={branchesList.length || 0} icon={Building2} iconColor="text-emerald-600" iconBg="bg-emerald-50" href="/branches" />
        <StatCard label="Open Maintenance" value={openMaintenance.length || 0} icon={Wrench} iconColor="text-amber-600" iconBg="bg-amber-50" href="/maintenance" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-card p-6">
          ...
          <div className="space-y-4">
            <QualityBar label="Assets with serial numbers" current={Number(summary.totalAssets ?? 0) - Number(summary.missingSerialNumber ?? 0)} total={Number(summary.totalAssets ?? 0)} color="bg-emerald-500" />
            <QualityBar label="Assets with purchase dates" current={Number(summary.totalAssets ?? 0) - Number(summary.missingPurchaseDate ?? 0)} total={Number(summary.totalAssets ?? 0)} color="bg-brand-500" />
            <QualityBar label="Assets with categories" current={Number(summary.totalAssets ?? 0) - Number(summary.missingCategory ?? 0)} total={Number(summary.totalAssets ?? 0)} color="bg-purple-500" />
            <QualityBar label="Recognized assets" current={Number(rec.capitalized ?? 0)} total={Number(summary.totalAssets ?? 0)} color="bg-amber-500" />
          </div>
