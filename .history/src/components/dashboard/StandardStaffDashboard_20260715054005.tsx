...
export function StandardStaffDashboard() {
  ...
  // Extract asset array: { data: PaginatedResponse<Asset> }
  const assets = (assignedAssets?.data?.data ?? []) as any[]
  // Notifications already unwrapped: PaginatedResponse<Notification>
  const notificationsList = (notifications?.data ?? []) as any[]
