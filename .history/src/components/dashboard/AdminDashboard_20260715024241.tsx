...
export function AdminDashboard() {
  ...
  const rec = summary?.recognitionSummary
  ...
  const missingDataAlerts = [
    { label: 'Missing serial numbers', count: summary?.missingSerialNumber ?? 0 },
    { label: 'Missing purchase dates', count: summary?.missingPurchaseDate ?? 0 },
    { label: 'Missing categories', count: summary?.missingCategory ?? 0 },
  ].filter((a) => a.count > 0)

  const treatmentData = [
    { name: 'Capitalized', value: rec?.capitalized ?? 0, color: '#8b5cf6' },
    { name: 'Expensed', value: rec?.expensed ?? 0, color: '#64748b' },
    { name: 'Tracked', value: rec?.tracked_non_capitalized ?? 0, color: '#3b82f6' },
    { name: 'Pending', value: rec?.pending_review ?? 0, color: '#f59e0b' },
  ].filter((d) => d.value > 0)
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Total Assets" value={summary?.totalAssets ?? 0} icon={Package} iconColor="text-brand-600" iconBg="bg-brand-50" href="/assets" />
        ...
        <StatCard label="Disposed Assets" value={summary?.disposedCount ?? 0} icon={TrendingDown} iconColor="text-red-600" iconBg="bg-red-50" />
      </div>
