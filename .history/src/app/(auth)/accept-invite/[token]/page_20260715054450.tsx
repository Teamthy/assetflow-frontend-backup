...
const roleCapabilities: Record<string, string[]> = {
  ...
  standard_staff: ['View assigned assets', 'Report maintenance needs', 'Track inventory', 'View personal tasks'],
}
...
export default function AcceptInvitePage({ params }: { params: Promise<{ token: string }> }) {
  ...
  async function onSubmit(values: AcceptForm) {
    ...
    try {
        ...
        user: {
          ...
          fullName:
            [response.user.firstName, response.user.lastName]
              .filter(Boolean)
              .join(' ') ||
            ((response.user as Record<string, unknown>).fullName as string) ||
            response.user.email ||
            '',
      ...
      router.push('/dashboard?welcome=true&source=invite')
    } catch {
  ...
  return (
    <div>
      ...
      {invitation && roleCapabilities[invitation.role] && (
        <div className="mb-6 rounded-xl border border-emerald-200 bg-emerald-50/50 p-4">
          <p className="text-xs font-semibold text-emerald-900 mb-2.5">What you'll be able to do:</p>
          <ul className="grid grid-cols-2 gap-2">
            {roleCapabilities[invitation.role].map((capability) => (
              <li key={capability} className="text-xs text-emerald-700 flex items-start gap-1.5">
