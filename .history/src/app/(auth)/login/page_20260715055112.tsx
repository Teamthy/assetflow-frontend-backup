  ...
  const onSubmit = async (data: LoginForm) => {
    ...
    try {
      ...
      let role: UserRole =
        ...
        responseData?.role ??
        'primary_admin'  // Default to primary_admin if not in response
      ...
      // Set auth with the role from response
      setAuth({
      ...
      // If role wasn't in the response, try to fetch it from team members
      if (!responseData?.member?.role && !responseData?.role) {
        try {
          const membersResponse = await settingsApi.getTeamMembers()
          const members = membersResponse.data?.data ?? []
          
          // Find current user in members list by email or ID
          ...
          // Fall back to the default role if fetch fails
