...
export default function OnboardingPage() {
  ...
  useEffect(() => {
    if (!isFirstLogin) router.replace('/dashboard')
  }, [isFirstLogin, router])
...
function StepContent({ step, onNext, onBack, onSkip, onFinish }: {
  step: number
  onNext: () => void
  onBack: () => void
  onSkip: () => void
  ...
  if (step === 4) {
    return (
      <div className="space-y-6">
        <div>
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white">{titles[3]}</h2>
          <p className="text-slate-500 mt-2 text-sm">{descriptions[3]}</p>
        </div>
        <div className="space-y-4">
          {[
            { emoji: '📊', label: 'Import from Excel', badge: 'RECOMMENDED', desc: 'Already have an asset register in Excel? Upload it in minutes.', action: () => { onNext(); window.location.href = '/assets/import?from=onboarding' } },
            { emoji: '✏️', label: 'Add assets manually', badge: null, desc: 'Start fresh and add assets one by one.', action: () => { onNext(); window.location.href = '/assets/new?from=onboarding' } },
