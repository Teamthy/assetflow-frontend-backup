...
export default function AuditReportPage() {
  ...
  const stats = useMemo(() => {
    const summary = (audit && typeof audit === 'object' && 'summary' in audit && audit.summary && typeof audit.summary === 'object'
      ? (audit.summary as Record<string, unknown>)
      : {}) as Record<string, unknown>

    const total = Number(summary.totalAssets ?? 0)
    const missingSerials = Number(summary.missingSerialNumber ?? 0)
    const missingDates = Number(summary.missingPurchaseDate ?? 0)
    const missingCategories = Number(summary.missingCategory ?? 0)
    const disposed = Number(summary.disposedCount ?? 0)
    const maintenance = Number(summary.maintenanceCount ?? 0)
    ...
    const recognitionSummary = (summary.recognitionSummary && typeof summary.recognitionSummary === 'object'
