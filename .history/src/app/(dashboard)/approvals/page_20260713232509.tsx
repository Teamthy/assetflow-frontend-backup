...
import { usePendingApprovals, useDecideDisposal } from '@/lib/hooks/useAdmin'

export default function ApprovalsPage() {
  const { data: approvals = [], isLoading, isError } = usePendingApprovals()
  const decideDisposal = useDecideDisposal('')
  ...
  async function handleDecision(approvalId: string, decision: 'approved' | 'rejected') {
    ...
    try {
      await decideDisposal.mutateAsync({ decision, decisionReason: decision === 'approved' ? 'Approved via dashboard' : 'Rejected via dashboard' })
    } catch {
              ...
              <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                ...
                <div className="flex gap-2">
                  <Button
                    ...
                    disabled={decisioningId === approval.id || decideDisposal.isPending}
                  >
