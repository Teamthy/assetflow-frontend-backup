...
function normalizeMembers(input: unknown): TeamMember[] {
  ...
  return items.map((item: Record<string, unknown>, index: number) => ({
    id: String(item.id ?? item.userId ?? item.user?.id ?? `${index}`),
    fullName: String(item.fullName ?? item.name ?? [item.firstName, item.lastName].filter(Boolean).join(' ') ?? item.user?.fullName ?? item.user?.firstName ?? item.email ?? 'Team member'),
    email: String(item.email ?? item.user?.email ?? ''),
    role: String(item.role ?? item.user?.role ?? 'standard_staff'),
    joinedAt: item.joinedAt ? String(item.joinedAt) : item.createdAt ? String(item.createdAt) : undefined,
    isCurrentUser: Boolean(item.isCurrentUser ?? item.user?.isCurrentUser ?? false),
...
function normalizeInvites(input: unknown): PendingInvite[] {
  ...
  return items.map((item: Record<string, unknown>, index: number) => ({
    id: String(item.id ?? `${item.email ?? 'invite'}-${index}`),
    email: String(item.email ?? ''),
    role: String(item.role ?? 'standard_staff'),
    sentAt: String(item.sentAt ?? item.createdAt ?? new Date().toISOString()),
