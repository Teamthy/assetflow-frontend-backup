...
for i in range(2):
    ...
    if s in (200, 201):
        ...
        if uid:
            ...
            print(f'   ⚠ No ID in user: {list(user.keys()) if isinstance(user, dict) else user}')
    else:
        print(f'   ✗ Failed: {str(b)[:100]}')
...
try:
    ...
    # Debug: Read back the Excel file and print what's in it
    print('\n   Debugging Excel content:')
    from openpyxl import load_workbook
    debug_wb = load_workbook('test-assets-import.xlsx')
    debug_ws = debug_wb.active
    for row_idx, row in enumerate(debug_ws.iter_rows(values_only=False), 1):
        if row_idx <= 2:  # Just check header and first data row
            print(f'   Row {row_idx}:')
            for col_idx, cell in enumerate(row, 1):
                val = cell.value
                dtype = type(val).__name__
                print(f'     Col {col_idx}: {repr(val)} (type={dtype})')
