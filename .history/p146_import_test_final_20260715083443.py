...
if s not in (200, 201):
    print(f'   FAIL: Registration failed: {b}')
    exit(1)
...
for i in range(2):
    ...
    if s in (200, 201):
        ...
        if uid:
            ...
            print(f'   WARN: No ID in user: {list(user.keys()) if isinstance(user, dict) else user}')
    else:
        print(f'   FAIL: Failed: {str(b)[:100]}')
...
try:
    ...
    print('   OK: Generated test-assets-import.xlsx with 5 sample assets')
    print(f'   Branch IDs used: {list(branch_ids.values())}')
    print(f'   Assignee IDs used: {list(assignee_ids.values())}')
    
except Exception as e:
    print(f'   FAIL: Failed to generate Excel: {e}')
    exit(1)
...
if s in (200, 201):
    ...
    if inserted > 0:
        ...
        print('\nWARN: No assets imported. Showing failures:')
