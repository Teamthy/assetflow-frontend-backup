...
export function DepreciationModal({ asset, open, onOpenChange }: Props) {
  ...
  async function onSubmit(values: RecordDepreciationFormValues) {
    if (!calc.totalMatches) {
      form.setError('totalAccumulatedDepreciation', {
        message: `Total must equal B/F + Yearly Charge = ${formatCurrency(calc.total)}`,
      })
      return
    }
    try {
      await recordMutation.mutateAsync({
        ...
        accumulatedDepreciationBf: Number(values.accumulatedDepreciationBf ?? 0),
        yearlyDepCharge: Number(values.yearlyDepCharge ?? 0),
        totalAccumulatedDepreciation: Number(values.totalAccumulatedDepreciation ?? 0),
