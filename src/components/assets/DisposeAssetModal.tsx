...
import { useRequestDisposalApproval } from '@/lib/hooks/useAdmin'
...
export function DisposeAssetModal({ asset, open, onOpenChange }: Props) {
  ...
  const requestApproval = useRequestDisposalApproval(asset.id)
  ...
  async function onSubmit(values: DisposeAssetFormValues) {
    ...
    try {
      if (requiresApproval) {
        await requestApproval.mutateAsync(payload)
      } else {
            ...
            <div className="text-xs text-amber-800">
              ...
              {requiresApproval && (
                <p className="mt-1">
                  ...
                  {formatCurrency(APPROVAL_THRESHOLD)} threshold. This will be
                  submitted for approval, not disposed immediately.
                </p>
              )}
            </div>
          </div>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)}>
            ...
            <DialogFooter className="px-6 py-4 border-t border-slate-100 bg-slate-50/50">
              ...
              >
                ...
                {requiresApproval ? 'Submit for approval' : 'Dispose Asset'}
              </Button>
            </DialogFooter>
          </form>
        </Form>
