...
export const approvalsApi = {
  ...
  decideDisposal: async (approvalId: string, data: ApprovalDecisionDto): Promise<Approval> => {
    const r = await apiClient.patch<unknown>('/approvals/' + approvalId + '/disposal/decision', {
      approved: data.approved ?? data.decision === 'approved',
      notes: data.notes ?? data.decisionReason,
  ...
  decideTransfer: async (approvalId: string, data: ApprovalDecisionDto): Promise<Approval> => {
    const r = await apiClient.patch<unknown>('/approvals/' + approvalId + '/transfer/decision', {
      approved: data.approved ?? data.decision === 'approved',
      notes: data.notes ?? data.decisionReason,
      decision: data.decision,
      decisionReason: data.decisionReason,
